...
% Si no hay tutores, definir macro vacía para evitar error en \@maketitle
...
\begin{itemize}
	...
	\item \textbf{Estabilidad:} El cuaderno muestra que es un sistema con un punto de silla inestable (uno de los autovalores tiene parte real positiva, según el análisis llevado a cabo).
\end{itemize}

\noindent\textbf{Representación matricial}

En el cuaderno se propone la siguiente representación matricial (con la notación usada en el notebook):
\[
\begin{bmatrix}
Q' \\
I'
...
Tomándolo como \[X' = A X\] con:
\[
A = \begin{bmatrix} 1 & 0 \\ -2 & -5 \end{bmatrix}, \quad X = \begin{bmatrix} I \\ Q \end{bmatrix}.
\]
...
Los autovectores asociados se hallan resolviendo $(A-\lambda I)v=0$; en el cuaderno se dan los siguientes vectores:
\[
v_{\lambda_1} = \begin{bmatrix} -3 \\ 1 \end{bmatrix}, \qquad v_{\lambda_2}=\begin{bmatrix} 0 \\ 1 \end{bmatrix}.
\]

La solución general, de acuerdo con la diagonalización propuesta, es
\[
X(t) = c_1 v_{\lambda_1} e^{\lambda_1 t} + c_2 v_{\lambda_2} e^{\lambda_2 t}.
\]
...
\end{cases}
\]
De esto se deduce $Q=0$ e $I=0$, luego el punto crítico del sistema es $(0,0)$.
